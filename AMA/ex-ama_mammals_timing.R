...
type = 2
for (i in 1:nRuns) {
  times_ama_fast_l2[i] = system.time({mammals_ama_fast_l2 = convex_cluster_path_fista_backtrack(X,w,gamma_seq_l2,nu=nu,tol=tol,type=type)})[3]
...
type=1
for (i in 1:nRuns) {
  times_ama_fast_l1[i] = system.time({mammals_ama_fast_l1 = convex_cluster_path_fista_backtrack(X,w,gamma_seq_l1,nu=nu,tol=tol,type=type)})[3]
...
type = 2
for (i in 1:nRuns) {
  times_ama_slow_l2[i] = system.time({mammals_ama_slow_l2 = convex_cluster_path_backtrack(X,w,gamma_seq_l2,nu=nu,tol=tol,type=type)})[3]
...
type = 1
for (i in 1:nRuns) {
  times_ama_slow_l1[i] = system.time({mammal_ama_slow_l1 = convex_cluster_path_backtrack(X,w,gamma_seq_l1,nu=nu,tol=tol,type=type)})[3]
...
for (iGamma in 1:nGamma) {
  ...
  obj_ama_l2[iGamma] = loss_primalF(X, mammals_ama_fast_l2$UHx[[iGamma]],gamma=gamma,w=w,ix=ix,type=2)
}
...
for (iGamma in 1:nGamma) {
  ...
  obj_ama_l1[iGamma] = loss_primalF(X, mammals_ama_fast_l1$UHx[[iGamma]],gamma=gamma,w=w,ix=ix,type=type)
}
